...
This project provides a Docker container for [MKV Muxing Batch GUI](https://github.com/yaser01/mkv-muxing-batch-gui).
...
```shell
docker stop mkv-muxing-batch-gui
```

  2. Remove the container:
```shell
docker rm mkv-muxing-batch-gui
```
...
services:
  mkv-muxing-batch-gui:
    ...
    ports:
      - "5800:5800"
    volumes:
      - "/docker/appdata/mkv-muxing-batch-gui:/config:rw"
...
[Releases]: https://github.com/jlesage/docker-mkv-muxing-batch-gui/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/mkv-muxing-batch-gui/tags
...
```shell
docker pull jlesage/mkv-muxing-batch-gui
```
...
For Synology NAS users, follow these steps to update a container image:
  ...
  3.  In the search bar, type the name of the container (`jlesage/mkv-muxing-batch-gui`).
  4.  Select the image, click *Download*, and choose the `latest` tag.
  5.  Wait for the download to complete. A notification will appear once done.
  6.  Click *Container* in the left pane.
  7.  Select your MKV Muxing Batch GUI container.
...
container, it would proxy all HTTP requests for `mkv-muxing-batch-gui.domain.tld` to
...
upstream docker-mkv-muxing-batch-gui {
...
server {
	...
	server_name mkv-muxing-batch-gui.domain.tld;

	location / {
	        proxy_pass http://docker-mkv-muxing-batch-gui;
	}
...
server {
	...
	location = /mkv-muxing-batch-gui {return 301 $scheme://$http_host/mkv-muxing-batch-gui/;}
	location /mkv-muxing-batch-gui/ {
		proxy_pass http://docker-mkv-muxing-batch-gui/;
		# Uncomment the following line if your Nginx server runs on a port that
		# differs from the one seen by external clients.
		#port_in_redirect off;
		location /mkv-muxing-batch-gui/websockify {
			proxy_pass http://docker-mkv-muxing-batch-gui/websockify;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
		...
		# Needed when audio support is enabled.
		location /mkv-muxing-batch-gui/websockify-audio {
			proxy_pass http://docker-mkv-muxing-batch-gui/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of MKV Muxing Batch GUI integrated into Docker image. |
...
Any desktop notification generated by MKV Muxing Batch GUI is forwarded to the
